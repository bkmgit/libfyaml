...
static struct fy_type_info *fy_type_get_type_info(struct fy_type *ft)
{
	...
	if (ti->kind == FYTK_PTR || ti->kind == FYTK_TYPEDEF)
		goto out;
...
char *fy_type_generate_name_internal(struct fy_type *ft, const char *field, int genidx)
{
...
#define FPRINTF(_fmt, ...) \
	...
	depname = fy_type_generate_name_internal(ftd, marker, genidx + 1);
	if (!depname)
		goto err_out;
	...
	case FYTK_PTR:
		if (ftd->type_kind != FYTK_FUNCTION)
			FPRINTF("%.*s*%s%s", (int)(ms - depname), depname, field, me);
		else
			FPRINTF("%.*s(*%s)%s", (int)(ms - depname), depname, field, me);
		break;
	...
	case FYTK_CONSTARRAY:
		if (ftd->type_kind != FYTK_FUNCTION)
			FPRINTF("%.*s%s[%llu]%s", (int)(ms - depname), depname, field, ft->element_count, me);
		else
			FPRINTF("%.*s(%s[%llu])%s", (int)(ms - depname), depname, field, ft->element_count, me);
		break;
...
char *fy_type_generate_name(struct fy_type *ft, const char *field)
{
	return fy_type_generate_name_internal(ft, field ? field : "", 0);
}
...
void fy_type_dump(struct fy_type *ft, bool no_location)
{
	...
	gen_type_name = fy_type_generate_name(ft, NULL);
	if (gen_type_name && strcmp(gen_type_name, ft->name)) {
		printf(":!'%s'", gen_type_name);
	}
	free(gen_type_name);
...
fy_reflection_lookup_type(struct fy_reflection *rfl, enum fy_type_kind type_kind, const char *name)
{
	...
	while (isblank(*type_name))
		type_name++;
	len = strlen(name);
	if (len > 7 && !strncmp(name, "struct ", 7))
		type_kind_auto = FYTK_STRUCT;
	else if (len > 6 && !strncmp(name, "union ", 6))
...
fy_type_info_generate_name(const struct fy_type_info *ti, const char *field)
{
	...
	return fy_type_generate_name(ft, field);
}
