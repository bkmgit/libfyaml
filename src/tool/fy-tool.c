	...
	for (i = 0, fi = ti->fields; i < ti->count; i++, fi++) {
		...
		if (!(fi->type_info->flags & FYTIF_ANONYMOUS)) {
			...
			if (ti->kind == FYTK_ENUM) {
				...
				name = fy_type_info_generate_name(fi->type_info, fi->name);
				assert(name);
				if (!(fi->flags & FYFIF_BITFIELD)) {
					printf("%s;", name);
...
static void type_info_c_typedef_dump(const struct fy_type_info *ti, int level)
{
	...
	if (!(ti->dependent_type->flags & FYTIF_ANONYMOUS)) {
		name = fy_type_info_generate_name(ti->dependent_type, ti->name);
		assert(name);
		printf("%s;", name);
		free(name);
		printf("\t/* size=%zu, align=%zu */", ti->size, ti->align);
...
reflection_lookup_type_by_fullname(struct fy_reflection *rfl, const char *fullname)
{
	const struct fy_type_info *ti;
	void *prev = NULL;
	...
	prev = NULL;
	while ((ti = fy_type_info_iterate(rfl, &prev)) != NULL) {
		if (!strcmp(ti->fullname, fullname))
			return ti;
	}
	return NULL;
}
...
int main(int argc, char *argv[])
{
	...
	case OPT_REFLECT:
		...
		if (type_dump)
			...
			rc = reflection_decoder_parse(rd, fyp, reflection_lookup_type_by_fullname(rfl, entry_type), NULL, 0);
			if (rc) {
